...
def main():
	download_myfitnesspal()
	process_myfitnesspal()

def process_myfitnesspal():
	(basepath/"data/myfitnesspal").mkdir(exist_ok=True, parents=True)
	(basepath/"data/myfitnesspal/cleaned").mkdir(exist_ok=True)
	(basepath/"data/myfitnesspal/processed").mkdir(exist_ok=True)

	entries = []
	with open(basepath/"data/myfitnesspal/rawdata/mfp-diaries.tsv", "r") as f:
		for line in f:
			line = line.strip()
			if not line:
				break
	...
	with open(basepath/"data/myfitnesspal/cleaned/myfitnesspal.json", "w") as f:
	...
	df.to_csv(basepath/"data/myfitnesspal/processed/myfitnesspal_foods.csv", index=False)
	...
	df.to_csv(basepath/"data/myfitnesspal/processed/myfitnesspal_users.csv", index=False)
...
def download_myfitnesspal():
	if (basepath/"data/myfitnesspal/rawdata/mfp-diaries.tsv").exists():
		return
	kaggle.api.authenticate()
	kaggle.api.dataset_download_files(
		"zvikinozadze/myfitnesspal-dataset",
		path=basepath/"data/myfitnesspal/rawdata/",
		unzip=True,
	)
